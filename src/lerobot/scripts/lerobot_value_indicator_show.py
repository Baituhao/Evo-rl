...
VIZ_TARGET_FIELD = "complementary_info.value_target"
VIZ_REWARD_FIELD = "complementary_info.value_reward"
...
def _export_visualization_outputs(
    ...
    viz_indicator_field = "complementary_info_acp_indicator_pistar06"
    if viz_indicator_field not in column_names:
        viz_indicator_field = cfg.acp.indicator_field
    written_videos = _export_overlay_videos(
        dataset=dataset,
        value_field=cfg.acp.value_field,
        advantage_field=cfg.acp.advantage_field,
        indicator_field=viz_indicator_field,
        ...
        draw_indicator_persistent_marker=True,
        draw_indicator_status_text=True,
        target_field=VIZ_TARGET_FIELD if VIZ_TARGET_FIELD in column_names else None,
        reward_field=VIZ_REWARD_FIELD if VIZ_REWARD_FIELD in column_names else None,
    )
...
def run_value_inference_pipeline(
    ...
    column_names = set(raw_frames.column_names)
    ...
    value_field_exists = cfg.acp.value_field in column_names
    if value_field_exists:
        predicted_values = _to_1d_float(raw_frames[cfg.acp.value_field])
        read_only_fields.add(cfg.acp.value_field)
        if accelerator.is_main_process:
            logging.info(
                "Value field '%s' already exists in dataset; reusing it in read-only mode.",
                cfg.acp.value_field,
            )
    else:
        ...
        checkpoint_str = str(pretrained_dir)
        ...
        if accelerator.is_main_process:
            ...
            write_fields.add(cfg.acp.value_field)

    if accelerator.is_main_process:
        if predicted_values is None:
            raise RuntimeError("Predicted values unexpectedly missing on main process.")
        ...
        columns: dict[str, np.ndarray] = {}
        feature_infos: dict[str, dict[str, Any]] = {}
        if cfg.acp.value_field in write_fields:
            columns[cfg.acp.value_field] = predicted_values.astype(np.float32)
            feature_infos[cfg.acp.value_field] = {"dtype": "float32", "shape": (1,), "names": None}

        indicator_positive_ratio: float | None = None
        thresholds: dict[int, float] | None = None
        viz_only_columns: dict[str, np.ndarray] = {}

        if cfg.acp.enable:
            if checkpoint_str is None:
                checkpoint_str = str(
                    _resolve_pretrained_model_dir(
                        checkpoint_path=cfg.inference.checkpoint_path,
                        checkpoint_ref=cfg.inference.checkpoint_ref,
                    )
                )
            value_cfg = PreTrainedConfig.from_pretrained(checkpoint_str)
            if not isinstance(value_cfg, Pistar06Config):
                raise ValueError(
                    f"Unsupported value config type '{type(value_cfg)}'. lerobot-value-infer currently supports only 'pistar06'."
                )
            ...
            advantage_field_exists = cfg.acp.advantage_field in column_names
            indicator_field_exists = cfg.acp.indicator_field in column_names

            if advantage_field_exists:
                advantages = _to_1d_float(raw_frames[cfg.acp.advantage_field])
                read_only_fields.add(cfg.acp.advantage_field)
            else:
                write_fields.add(cfg.acp.advantage_field)

            if indicator_field_exists:
                indicators = _to_1d_int(raw_frames[cfg.acp.indicator_field])
                read_only_fields.add(cfg.acp.indicator_field)
            else:
                ...
                write_fields.add(cfg.acp.indicator_field)
            ...
            if cfg.acp.advantage_field in write_fields:
            ...
            if cfg.acp.indicator_field in write_fields:
            ...
            viz_only_columns[VIZ_TARGET_FIELD] = value_targets.astype(np.float32)
            viz_only_columns[VIZ_REWARD_FIELD] = rewards.astype(np.float32)
        ...
        if columns:
            ...
            logging.info(
                "Wrote annotation fields to dataset root: %s | fields=%s",
                dataset.root,
                sorted(columns),
            )
        else:
            logging.info(
                "All requested annotation fields already exist; dataset stays read-only for %s.",
                dataset.root,
            )

        # Sync computed columns into the in-memory hf_dataset so viz can read them
        for field, values in {**columns, **viz_only_columns}.items():
        ...
        result = {
            ...
            "thresholds": thresholds,
            "viz_outputs": viz_outputs,
            "read_only_fields": sorted(read_only_fields),
            "written_fields": sorted(columns),
        }
    else:
        result = {
            "main_process": False,
