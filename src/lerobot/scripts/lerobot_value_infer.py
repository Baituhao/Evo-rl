...
# DEPRECATED: This fake streaming function is no longer used.
# streaming_write=true now uses true streaming (value_infer_streaming.py)
# which processes episode-by-episode with checkpoint resume.
# This function accumulated all data in memory (all_advantages, all_task_indices)
# and only wrote at the end, providing no real memory benefit or fault tolerance.
#
# def _write_columns_sidecar_streaming(...):
#     """Write sidecar parquet with chunked processing to reduce memory peaks.
#     [DEPRECATED - See value_infer_streaming.py for true streaming implementation]
#     """
#     ...
...
def run_value_inference_pipeline(
    ...
    # Check if using streaming mode (completely bypasses normal inference loop)
    if cfg.acp.enable and cfg.acp.streaming_write and cfg.acp.write_mode == "sidecar":
        logging.info("Using streaming mode (episode-level with checkpoint resume)")
        ...
        # Import and run streaming inference with checkpoint resume
        ...
        # Streaming inference handles everything, return early
        result = {
    ...
    # Full-memory mode (non-streaming path)
    if accelerator.is_main_process:
    ...
    if accelerator.is_main_process:
        ...
        if cfg.acp.enable:
            ...
            # Full-memory mode: compute all at once
