...
PISTAR_06_TD_SAVE_INFO = "pistar_06_td_save_info.json"


# ─────────────────────────────────────────────────────────────────────────────
# TD Loss Design (RISE-inspired, offline bootstrap variant)
# ─────────────────────────────────────────────────────────────────────────────
# This value model combines two losses directly:
#   L = L_CE + td_loss_weight * L_TD
#
# 1. L_CE: Distributional cross-entropy over 201 bins (original pi*06 design).
#    Supervises the full return distribution against Monte-Carlo targets.
#
# 2. L_TD: Scalar MSE between the expected value V(s) and a Bellman bootstrap target:
#       y_t = r_t + gamma * V_MC(s_{t+1})
#    where:
#      - r_t is the dense per-step reward derived from consecutive MC targets,
#      - V_MC(s_{t+1}) is the dataset's own Monte-Carlo target at the next frame
#        (not a learned target critic; this is "offline bootstrap"),
#      - at episode boundaries (terminal frames) the bootstrap term is dropped: y_t = r_t.
#
# This differs from RISE's original design in one key aspect:
#   RISE uses an online EMA target network to compute V(s_{t+1}) during training.
#   Here we precompute y_t offline in the raw-batch hook using the dataset's MC targets,
#   avoiding the need for a target network, next-frame observations in the batch,
#   or online forward passes for V(s').
#
# The tradeoff: bootstrap values come from a fixed Monte-Carlo estimator rather than
# the model's own learned critic, so the TD signal does not adapt as the value function
# improves. This is simpler to implement and requires minimal changes to the dataloader.
# ─────────────────────────────────────────────────────────────────────────────
...
class Pistar_06_tdModel(nn.Module):
    def __init__(self, cfg: Pistar_06_tdConfig):
        super().__init__()
    ...
    def forward(
        ...
        # Handle temporal dimension: [B, N_delta, N_cam, C, H, W] or [B, N_cam, C, H, W]
        if images.ndim == 6:
        ...
        if images.shape[1] == 0:
            raise ValueError("At least one camera is required for Pistar_06_tdModel.")
...
class Pistar_06_tdPolicy(PreTrainedPolicy):
    config_class = Pistar_06_tdConfig
    name = "pistar_06_td"
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.config = config
        self.model = Pistar_06_tdModel(config)
        ...
        # Create target model for RISE-style online TD bootstrap
    ...
    def _save_pretrained(self, save_directory: Path) -> None:
        ...
        with open(save_directory / PISTAR_06_TD_SAVE_INFO, "w", encoding="utf-8") as f:
    ...
    def _load_save_info(
        ...
        *,
        ...
        if os.path.isdir(model_id):
            candidate = Path(model_id) / PISTAR_06_TD_SAVE_INFO
            ...
            try:
                resolved = hf_hub_download(
                    repo_id=model_id,
                    filename=PISTAR_06_TD_SAVE_INFO,
                    revision=revision,
                    cache_dir=cache_dir,
    ...
    def from_pretrained(
        ...
        pretrained_name_or_path: str | Path,
        *,
        config: Pistar_06_tdConfig | None = None,
        force_download: bool = False,
        resume_download: bool | None = None,
        proxies: dict | None = None,
        token: str | bool | None = None,
        ...
        **kwargs: Any,
        ...
        if save_info is not None and save_info.get("weights_mode") == "partial":
            if strict:
                logging.info(
                    "Detected partial Pistar_06_td checkpoint at '%s'; "
    ...
    def predict_action_chunk(self, batch: dict[str, Tensor], **kwargs: ActionSelectKwargs) -> Tensor:
        raise RuntimeError("Pistar_06_tdPolicy is a value model and does not support action prediction.")

    def select_action(self, batch: dict[str, Tensor], **kwargs: ActionSelectKwargs) -> Tensor:
        raise RuntimeError("Pistar_06_tdPolicy is a value model and does not support action selection.")

    def predict_value(self, batch: dict[str, Tensor]) -> Tensor:
        input_ids = batch[OBS_LANGUAGE_TOKENS]
        attention_mask = batch[OBS_LANGUAGE_ATTENTION_MASK]
        images = batch[PISTAR_06_TD_IMAGES_KEY]
        image_attention_mask = batch[PISTAR_06_TD_IMAGE_MASK_KEY]
    ...
    def build_training_raw_batch_hook(self, dataset, targets_cfg):
        ...
        # Per-frame lookups needed by the RISE-style online TD loss.
        # episode_length is episode-level metadata and is otherwise absent from the batch,
        # so we materialize a per-absolute-index lookup here. frame_index is re-injected too
        # so the forward pass does not depend on it surviving the preprocessor pipeline.
        episode_length_lookup = np.zeros(max_index + 1, dtype=np.int64)
        frame_index_lookup = np.zeros(max_index + 1, dtype=np.int64)
        # is_failure_lookup: dataset has no success/failure label -> treat every episode as
        # successful (all zeros). When a label becomes available, fill this from episode_info.
        is_failure_lookup = np.zeros(max_index + 1, dtype=np.float32)
        for pos in range(episode_indices.shape[0]):
            abs_idx = int(absolute_indices[pos])
            ep = episode_info[int(episode_indices[pos])]
            episode_length_lookup[abs_idx] = ep.length
            frame_index_lookup[abs_idx] = int(frame_indices[pos])
            if not ep.success:
                is_failure_lookup[abs_idx] = 1.0
        ...
        def value_target_hook(batch: dict[str, Any], step: int) -> dict[str, Any]:
            ...
            # Inject the per-frame fields the online TD loss needs.
    ...
    def forward(self, batch: dict[str, Tensor], reduction: str = "mean") -> tuple[Tensor, dict]:
        ...
        # Detect dual-frame mode: when delta_indices is enabled, processor produces:
        # - images: [B, 2, N_cam, C, H, W] (unified dual-frame, current + next)
        # - input_ids/attention_mask: [2B, T] (concatenated: first B are current, last B are next)
        # Split them for V(s) (current) and V_target(s') (next).
        ...
        if is_dual_frame:
            ...
            input_ids_next = input_ids[B:]
            attention_mask_current = attention_mask[:B]
            attention_mask_next = attention_mask[B:]

            # Use current-frame inputs for main model V(s)
            input_ids = input_ids_current
            attention_mask = attention_mask_current
            images = images_current
        else:
            # Single-frame mode: no splitting needed
            images_next = None
            input_ids_next = None
            attention_mask_next = None
        ...
        # Cross-entropy component, captured before the TD term is folded into per_sample_loss.
        ce_loss_value = per_sample_loss.mean()

        # TD loss (RISE-style online bootstrap with target network).
        # Computes sparse rewards at episode termination and bootstraps from target model's V(s').
        td_loss_value = None

        # Debug: log why TD loss might be skipped
        if self.config.td_loss_weight <= 0:
            pass  # TD loss disabled in config
        ...
        if self.config.td_loss_weight > 0 and self.target_model is not None and is_dual_frame:
            ...
            if frame_index is not None and episode_length is not None:
                if not isinstance(frame_index, Tensor):
                    frame_index = torch.as_tensor(frame_index, device=device)
                if not isinstance(episode_length, Tensor):
                    episode_length = torch.as_tensor(episode_length, device=device)
                if not isinstance(is_failure, Tensor):
                    is_failure = torch.as_tensor(is_failure, device=device)

                frame_index = frame_index.to(device=device, dtype=torch.long)
                episode_length = episode_length.to(device=device, dtype=torch.long)
                is_failure = is_failure.to(device=device, dtype=torch.float32)

                # Terminal window: last N frames of episode
                is_terminal = (episode_length - frame_index) <= self.config.td_terminal_window
                ...
                # Predict next value using target model with next-frame inputs (s', state')
                with torch.no_grad():
                    ...
                    bin_centers_device = self.bin_centers.to(device=device)
                    next_value = expected_value_from_logits(next_logits, bin_centers_device)

                # Bellman backup: y = r + gamma * (1 - done) * V_target(s')
        ...
        loss_dict = {
            ...
            "loss_ce": float(ce_loss_value.detach().item()),
            "value_mae": float(value_mae.detach().item()),
        }
        if td_loss_value is not None:
            loss_dict["loss_td"] = float(td_loss_value.detach().item())
        if sample_weight is not None:
