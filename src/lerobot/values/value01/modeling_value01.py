...
from lerobot.values.value01.processor_value01 import VALUE01_IMAGE_MASK_KEY, VALUE01_IMAGES_KEY
...
VALUE01_SAVE_INFO = "value01_save_info.json"
...
def compute_normalized_value_targets(
    ...
    *,
    ...
    for i in range(episode_indices.shape[0]):
        ...
        c_fail = float(frame_indices[i]) * c_fail_coef
        g = - float(remaining_steps) * 0.5
        
        if not ep.success:
            g -= c_fail

        denom = float(ep.length) - 1
        g_norm = g / denom
    ...
    for i in range(n):
        ...
        if is_next_in_episode:
            ...
            if ep_idx not in episode_info:
                raise KeyError(f"Missing episode metadata for episode_index={ep_idx}.")
            ep = episode_info[ep_idx]
            rewards[i] = float(targets[i]) if ep.success else float(targets[i]) + 1.0
    ...
    for i in range(n):
        ...
        if steps == n_step and j < n and episode_indices[j] == ep_i and frame_indices[j] == fi + n_step:
            ...
            if ep_idx not in episode_info:
                raise KeyError(f"Missing episode metadata for episode_index={ep_idx}.")
            ep = episode_info[ep_idx]
            bootstrap = 0.0 if ep.success else -1.0
...
class Value01Model(nn.Module):
    def __init__(self, cfg: Value01Config):
        super().__init__()
    ...
    def forward(
        ...
        if images.shape[1] == 0:
            raise ValueError("At least one camera is required for Value01Model.")
...
class Value01Policy(PreTrainedPolicy):
    config_class = Value01Config
    name = "value01"

    def __init__(
        self,
        config: Value01Config,
        dataset_meta=None,
        **kwargs: Any,
    ):
        ...
        self.config = config
        self.model = Value01Model(config)
    ...
    def _save_pretrained(self, save_directory: Path) -> None:
        ...
        with open(save_directory / VALUE01_SAVE_INFO, "w", encoding="utf-8") as f:
    ...
    def _load_save_info(
        ...
        *,
        ...
        if os.path.isdir(model_id):
            candidate = Path(model_id) / VALUE01_SAVE_INFO
    ...
    def from_pretrained(
        ...
        pretrained_name_or_path: str | Path,
        *,
        config: Value01Config | None = None,
        force_download: bool = False,
        resume_download: bool | None = None,
        proxies: dict | None = None,
        token: str | bool | None = None,
        ...
        **kwargs: Any,
        ...
        if save_info is not None and save_info.get("weights_mode") == "partial":
            if strict:
                logging.info(
                    "Detected partial Value01 checkpoint at '%s'; "
    ...
    def predict_action_chunk(self, batch: dict[str, Tensor], **kwargs: ActionSelectKwargs) -> Tensor:
        raise RuntimeError("Value01Policy is a value model and does not support action prediction.")

    def select_action(self, batch: dict[str, Tensor], **kwargs: ActionSelectKwargs) -> Tensor:
        raise RuntimeError("Value01Policy is a value model and does not support action selection.")

    def predict_value(self, batch: dict[str, Tensor]) -> Tensor:
        input_ids = batch[OBS_LANGUAGE_TOKENS]
        attention_mask = batch[OBS_LANGUAGE_ATTENTION_MASK]
        images = batch[VALUE01_IMAGES_KEY]
        image_attention_mask = batch[VALUE01_IMAGE_MASK_KEY]
