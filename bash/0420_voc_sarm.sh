#!/bin/bash
TIMESTAMP=$(date +%Y%m%d_%H%M%S)
EXPERIMENT_DESC="实验描述：voc 计算 sarm voc 0424"
DATE="0424_voc_sarm"
STAGE1="voc_compute_hg_500" 
STAGE1_TAG="sarm_0410"

source /mnt/data/syk/.bashrc
conda activate evo-rl


export LEROBOT_LOG_LEVEL="INFO"  # set to INFO to disable ACP debug logs ; DEBUG to enable ACP debug logs


mkdir -p "outputs/${DATE}"
LOG_FILE="outputs/${DATE}/${STAGE1}_${STAGE1_TAG}_${TIMESTAMP}.log"
echo "[${TIMESTAMP}] ${EXPERIMENT_DESC}" >> "${LOG_FILE}"


echo "====================================================================================================" >> "${LOG_FILE}"
echo "${STAGE1} begin" >> "${LOG_FILE}"
echo "====================================================================================================" >> "${LOG_FILE}"



# voc compute
setsid nohup \
    python /mnt/data/syk/Evo-RL/analysis/compute_voc_from_sarm_progress.py \
    --progress-parquet "datasets/openarm_data_260306_260330_sft_style_hg/sarm_progress_2300_2799.parquet" \
    --progress-column "progress_sparse" \
    --episodes "2300-2799" \
    --output-dir "outputs/${DATE}/${STAGE1}_${STAGE1_TAG}_${TIMESTAMP}" \
    >> "${LOG_FILE}" 2>&1 &


echo "Started ${STAGE1} (logs: ${LOG_FILE})"
source bash/wait_monitor.sh

echo "====================================================================================================" >> "${LOG_FILE}"
echo "${STAGE1} completed" >> "${LOG_FILE}"
echo "====================================================================================================" >> "${LOG_FILE}"


